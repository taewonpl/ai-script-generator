 ...
 */
export type PreviewContent = string | { markdown: string; tokens?: number } | null | undefined;
 ...
 */
export function isMarkdownObject(content: PreviewContent): content is { markdown: string; tokens?: number } {
 ...
 */
export function extractMarkdown(content: PreviewContent, fallback = ''): string {
  if (!content) {
    return fallback;
  }
